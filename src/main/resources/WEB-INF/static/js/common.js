...
var util = {
	...
	postSync:function(url,data,callback){
		this.ajax({url:url, contentType:"application/json;charset=UTF-8", data:data,type:"post",async:false},callback);
	},
	...
	table:function(url,cols,option,where,select,id){
		...
		layui.form.on('submit(select)', function(data){
			...
			var res = {};
			if(where){
				res = Object.assign(where,data.field)
			}else{
				res = data.field
			}
	...
	isNull:function(data){
		return data.toString().replace(/\s/g, "").length <= 0;
	},
	form:{
		isNull:function(data){
			for(d in data){
		...
		len:function(data,start,end){
			...
			return len<start?true:(end?(len >= end):false)
		}
	},
	...
	getEditor:function(id,options){
		...
		return UE.getEditor(elementId, opt);
	},
